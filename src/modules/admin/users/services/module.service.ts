...
export class ModuleService extends BaseModuleService {
  ...
  async generateTOTP(txn: ITxn): Promise<TTotpResponse> {
    try {
      this.logger.debug(`Invoking "generateTOTP" method...`);
      const { props, content } = this.extract<TTotpRequest>(txn);
      const payload = await this.helperService.generateTOTP(props, content);
  ...
  async enableTOTP(txn: ITxn): Promise<TTotpResponse> {
    try {
      this.logger.debug(`Invoking "generateTOTP" method...`);
      const { props, content } = this.extract<TTotpRequest>(txn);
  ...
  async disableTOTP(txn: ITxn): Promise<TTotpResponse> {
    try {
      this.logger.debug(`Invoking "generateTOTP" method...`);
      const { props, content } = this.extract<TTotpRequest>(txn);
